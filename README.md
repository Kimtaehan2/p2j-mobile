...
일러스트는 `assets/illustrations/` 에 있다. `2.0x` / `3.0x` / `4.0x` 하위 폴더는
Flutter 가 화면 밀도에 맞춰 알아서 고른다. 원본(1024px)은 `masters-1024/` 에 두었고
앱에 번들되지 않는다 — 아이콘 생성과 나중 리터치용 소스다.

경로는 `AppIllustrations` 에 모아 둔다. 화면마다 문자열을 적으면 오타를 컴파일러가 못 잡는다.

| 그림 | 쓰는 곳 |
| --- | --- |
| `01_app_launcher` | 앱 아이콘 |
| `02_today_todo` | 오늘 할 일이 비었을 때 |
| `03_goal_add` | 목표가 비었을 때 |
| `04_voice_input` | 음성 입력 시트 |
| `05_ai_plan` | 인트로 — 계획은 저희가 관리할게요 |
| `06_load_adjust` | 인트로 — 계획 세우기 어려우신가요 · 통계 준비 중 |
| `07_group_share` | 그룹 준비 중 |
| `10_achievement_streak` | 인트로 — 함께 시작해봐요 · 오늘 다 끝냈을 때 |

`08_declaration_lock` 과 `09_proof_upload` 는 12~13주차 그룹 기능용이라 아직 쓰지 않는다.

앱 아이콘을 바꾸려면 `flutter_launcher_icons.yaml` 을 고치고 아래를 돌린다.
...
인트로에서 마스코트(`assets/images/mascot.png`)가 말풍선으로 말을 건다.
질문을 화면이 던지는 것과 캐릭터가 묻는 것은 다르게 읽힌다. 이 앱은 사용자에게
계획을 요구하지 않고 대신 세워 주는 앱이라, 첫 화면부터 사람 대하듯 말을 거는 쪽이 맞다.

키보드가 올라오면 마스코트를 접는다(`MediaQuery.viewInsetsOf`). 말풍선과 입력칸이 먼저다.

**어두운 화면에서 주요 버튼은 `AppTheme.filledOnEspresso()` 를 쓴다.**
기본 주요 버튼은 잉크색(`#211D16`)인데 인트로 배경(`#241E17`)과 밝기가 거의 같아
...
할 일 추가는 **말하는 게 기본**이다. 이 앱은 계획 세우는 일을 대신해 주는 앱이라,
사용자가 문장을 다듬어 입력할 필요가 없어야 한다. 타이핑("직접 쓸게요")은 빠져나갈 길로만 둔다.

`speech_to_text` 로 받아 적기만 한다. **받아 적은 문장을 여러 할 일로 쪼개는 건 서버 몫이다.**
지금은 문장을 그대로 제목으로 넘긴다. `/ai/parse` 가 붙으면 `voice_input_controller.dart` 에서
그쪽으로 보내면 된다.

안드로이드 설정 두 가지가 `AndroidManifest.xml` 에 들어 있다.

- `RECORD_AUDIO` 권한
- `android.speech.RecognitionService` 조회 — Android 11+ 는 패키지 조회를 막아서,
  이게 없으면 기기에 음성 인식기가 있어도 플러그인이 찾지 못한다

에뮬레이터에는 마이크 입력이 없어 실제 인식은 안 된다. 권한 요청과 듣기 화면까지만 확인된다.
실기기로 테스트한다. 테스트 중 권한을 미리 주려면 아래를 쓴다.

```
adb shell pm grant dev.p2j.p2j_mobile android.permission.RECORD_AUDIO
```
...
AI 파싱(`/ai/parse`), 계획량 안내(10주차),
그룹·선언·인증샷·랭킹(12~13주차), 통계 화면(11주차), 푸시·딥링크·다국어.

바텀 네비게이션의 그룹·통계 탭은 "곧 만나요" 플레이스홀더다.

음성 입력은 원래 8주차 작업이었으나 앞당겨 넣었다. 받아 적기까지만 되고
AI 파싱은 서버가 붙어야 한다.
